...
int main(int argc, char **argv) {
	...
	if (config.hasOutputDeviceName()) {
		std::cout << "**Abrindo dispositivo de saida.                        **"
				<< std::endl;
	...
	if (config.hasSensorDeviceName()) {
		...
		sensorsThread = StartSensorsThread(config.getSensorDeviceName().c_str(),
				sd);
	...
	*cap >> frame;
	...
	if (config.reconfigure()) {
		std::cout << std::endl
				<< "** Zerando, perspectiva, área de corte e area de interesse " << std::endl;
	...
	CycloConfig* cfg = &config;
	CycloTracker tracker(cfg, cap, outputFile, outputDevice, sd);
