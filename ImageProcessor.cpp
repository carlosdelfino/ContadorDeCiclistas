...
ImageProcessor::ImageProcessor(CycloConfig* config) {
	setCropArea(config->getCropArea());
	setPerspective(							//.
			config->GetPerspectivePos(0),	//.
			config->GetPerspectivePos(1),			//.
			config->GetPerspectivePos(2),			//.
			config->GetPerspectivePos(3)			//.
					);
	setInterestArea(config->getInterestArea());
...
void ImageProcessor::InsertInterestArea(cv::Mat &frame) {
	InsertInterestArea(frame, interestArea);
}
...
void ImageProcessor::PrepareFrame(cv::Mat &frame) {
	PrepareFrame(frame, cropArea, quad[0], quad[1], quad[2], quad[3]);
}

void ImageProcessor::PrepareFrame(cv::Mat &frame, cv::Rect cropArea,
		cv::Point2f p0, cv::Point2f p1, cv::Point2f p2, cv::Point2f p3) {
	//this->RotateImage(frame);
	this->PerspectiveTransform(frame, p0, p1, p2, p3);
	this->CropImage(frame, cropArea);
}

void ImageProcessor::setCropArea(cv::Rect cropArea) {
	this->cropArea = cropArea;
}

void ImageProcessor::setInterestArea(cv::Rect interestArea) {
	this->interestArea = interestArea;
}
void ImageProcessor::setPerspective(	//.
		cv::Point2f p0,					//.
		cv::Point2f p1, 					//.
		cv::Point2f p2, 					//.
		cv::Point2f p3					//.
...
void ImageProcessor::PerspectiveTransform(cv::Mat &frame, cv::Point2f p0,
	...
	y1d = 0;
	x2d = 0;
	y2d = std::max(p2.y - p0.y, p3.y - p1.y);
	x3d = p3.x;
	y3d = std::max(p2.y - p0.y, p3.y - p1.y);
